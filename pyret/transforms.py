...
def dct(x,axis=-1):
    ...
    if even:
        ...
        newshape[axis] = 2*N
        xtilde = scipy.empty(newshape,dtype=float)
    ...
    pk = scipy.exp(-1j*pi*scipy.arange(N)/(2*N))
    newshape = scipy.ones(n)
    newshape[axis] = N
    pk.shape = newshape
...
def idst(v,axis=-1):
    ...
    newshape = list(v.shape)
    newshape[axis] = 2*(N+1)
    Xt = scipy.zeros(newshape, dtype=complex)
    slices[0][axis] = slice(1,N+1)
    slices[1][axis] = slice(N+2,None)
    slices[2][axis] = slice(None,None,-1)
    val = 2j*v
    ...
    xhat = scipy.real(scipy.ifft(Xt,axis=axis))
